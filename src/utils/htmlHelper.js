  ...
  for (let index = 0; index < content.length; index++) {
      ...
      switch (state) {
          ...
          case TokenizerState.inEntity:
            if (c === ";") {
              result.push(new Token(tokenStart, index + 1, TokenType.entity, content.substring(tokenStart + 1, index)))
              tokenStart = index + 1;
              state = TokenizerState.default;
            } else {
...
const splitHtmlTextOnPages = (content, fontSizePx, pageInnerWidthPx, pageInnerHeightPx, document) => {
    ...
    const addNewPage = (newLineTokenIndex, endIndex) => {
      const pageText = content.substring(pageStartCharIndex, endIndex) + tagStack.reverse().map(it => `</${extractTagName(it)}>`).join("");
    ...
    const computeLetterSpacing = (c1, c2) => {
        ...
        const spacing = pairWidth / (fontSizeForComputation / fontSizePx) -
            computeLetterWidth(c1) - computeLetterWidth(c2);
    ...
    tokens.forEach((token, index) => {
      switch (token.type) {
        case TokenType.openingTag:
          if (token.tagName === "span" && content.substring(token.startIndex, token.endIndex).indexOf("data-page-break") >= 0) {
            addNewPage(index, token.startIndex);
          } else if (currentLineWidth > 0 && (token.tagName === "p" || token.tagName === "div")) {
        ...
        default:
          const newLineWidth = computeTokenizedTextWidth(content, tokens.slice(lineStartTokenIndex, index + 1));

          if (newLineWidth > pageInnerWidthPx) {
              addNewLine(index, token.startIndex);
              currentLineWidth = computeTokenizedTextWidth(content, tokens.slice(index, index + 1));
          } else {
