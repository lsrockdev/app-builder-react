...
export default class Resizable extends React.Component {
  ...
  onResizeStart(event, direction) {
    ...
    if (this.props.onResizeStart) {
      this.props.onResizeStart(
        event,
        direction,
        (this.resizable: React.ElementRef<"div">)
      );
    }
  ...
  onMouseMove(event) {
    ...
    maxHeight =
      typeof maxHeight === "undefined" ? undefined : Number(maxHeight);
    minWidth = typeof minWidth === "undefined" ? undefined : Number(minWidth);
    minHeight =
      typeof minHeight === "undefined" ? undefined : Number(minHeight);
    ...
    if (/right/i.test(direction)) {
      ...
      if (lockAspectRatio)
        newHeight =
          (newWidth - lockAspectRatioExtraWidth) / ratio +
          lockAspectRatioExtraHeight;
    }
    ...
    if (/bottom/i.test(direction)) {
      ...
      if (lockAspectRatio)
        newWidth =
          (newHeight - lockAspectRatioExtraHeight) * ratio +
          lockAspectRatioExtraWidth;
    }
    ...
    const computedMaxWidth =
      typeof maxWidth === "undefined" || maxWidth < 0 ? newWidth : maxWidth;
    const computedMinHeight = typeof minHeight === "undefined" ? 10 : minHeight;
    const computedMaxHeight =
      typeof maxHeight === "undefined" || maxHeight < 0 ? newHeight : maxHeight;

    if (lockAspectRatio) {
      const extraMinWidth =
        (computedMinHeight - lockAspectRatioExtraHeight) * ratio +
        lockAspectRatioExtraWidth;
      const extraMaxWidth =
        (computedMaxHeight - lockAspectRatioExtraHeight) * ratio +
        lockAspectRatioExtraWidth;
      const extraMinHeight =
        (computedMinWidth - lockAspectRatioExtraWidth) / ratio +
        lockAspectRatioExtraHeight;
      const extraMaxHeight =
        (computedMaxWidth - lockAspectRatioExtraWidth) / ratio +
        lockAspectRatioExtraHeight;
  ...
  get sizeStyle() {
    ...
    const getSize = (key: "width" | "height") => {
    ...
    const width =
      size && size.width && !this.state.isResizing
        ? getStringSize(size.width)
        : getSize("width");
    const height =
      size && size.height && !this.state.isResizing
